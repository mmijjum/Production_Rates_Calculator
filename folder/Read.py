...
Exposure_Age_Inputs = pd.read_excel('inputs.xlsx') #excel sheet with all pmag data. 0-70 Ma, all criteria
...
Sinx3HeT = pd.read_csv('Sinx3HeT', header=None) 
...
# # Muons
# - only for 21Ne qtz
mfluxRef_neg = pd.read_csv('mfluxRef_neg', header = None)
mfluxRef_pos = pd.read_csv('mfluxRef_pos', header = None)
...
u4n = 4.5141e-3;
u5n = 3.1992e8;
#Phimu = zeros(length(Rc),length(E)); make this later
Phimn_list = []
Phimp_list = []
