...
import numpy as np
import pandas as pd
import scaling_factor_MCADAM
import shielding
import Muons_v2_MCADAM
...
if Read.paleo[0] == 1:
    dt_start = float(Read.time[-1] - Read.stop)
...
for i in range(len(scaling_factor_MCADAM.Siteprod_df)): #how many samples 
    temp_start = (scaling_factor_MCADAM.Siteprod_df.iloc[i][0]* np.exp( (-erosion[i]*i)/lambdasp)* (dt_start*10**6))
...
if Read.muons == 'False': 
    ...
    for i in range(len(scaling_factor_MCADAM.Siteprod_df)): #how many samples 
        for j in range(len(scaling_factor_MCADAM.Siteprod_df.iloc[0])): #time length
            temp = (scaling_factor_MCADAM.Siteprod_df.iloc[i][j]* np.exp((-erosion[i]*i)/lambdasp)* dt)
    ...
    for i in range (len(tempvals_df)):
        ...
        dt2 = ((n0[i]/(sthick[i] *stopo[i]* slhl)) - a) / (scaling_factor_MCADAM.Siteprod_df.iloc[i][iteration[i]]* np.exp((-erosion[i]*i)/lambdasp))
        if iteration[i] == 0:
            expage = dt2
        else:
        ...
        # empirical muons part - based on Larsen et al. (2021)
        # Pmu = 0.23
        # lambdamu=8780
        ...
        # for i in range(len(scaling_factor_MCADAM.Siteprod_df)): #how many samples 
        #     for j in range(len(scaling_factor_MCADAM.Siteprod_df.iloc[0])): #time length
        #         temp = (scaling_factor_MCADAM.Siteprod_df.iloc[i][j]* dt)
        #         tempvals.append(temp)
        #       #   n = sthick * slhl * temp
        #       #   if n == n0:
...
if Read.muons == 'True': 
    ...
    Pmu = Muons_v2_MCADAM.pmuons_df
    firstmubin = []
    for i in range(len(scaling_factor_MCADAM.Siteprod_df)): #how many samples 
        for j in range(len(scaling_factor_MCADAM.Siteprod_df.iloc[0])):
    ...
    for i in range (len(tempvals_df)):
        a = (np.sum(tempvals_df.iloc[i][0:iteration[i]]))
        b = (np.sum(tempvals_df_mu.iloc[i][0:iteration[i]]))
        dt2 = (n0[i] - (sthick[i]*stopo[i]*slhl*a) - b)/ (sthick[i]*stopo[i]*slhl* (scaling_factor_MCADAM.Siteprod_df.iloc[i][iteration[i]]* np.exp((-erosion[i]*i)/lambdasp)) + Pmu.iloc[i][iteration[i]]* np.exp(((-erosion[i]*i)-shielding.z_df.iloc[i][iteration[i]]/2)/lambdamu.iloc[i][iteration[i]]))
        expage = ((iteration[i]-1) * dt) + (dt_start*10**6) + dt2
        exp_age.append(expage)
